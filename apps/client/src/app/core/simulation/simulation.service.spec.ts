...
describe('SimulationService', () => {
  ...
  it('Should give different registry based on locale', () => {
    const service: SimulationService = TestBed.get(SimulationService);
    const registry = service.getAllActions('en').map(a => a.name);
    const krRegistry = service.getAllActions('ko').map(a => a.name);
    const cnRegistry = service.getAllActions('zh').map(a => a.name);
