...
output('gathering-bonuses', db('GatheringPointBonus', false).toObject(row => {
  return +row.BonusValue ? {
    ...
    bonus: {
      ko: db('GatheringPointBonusType', null, true).findById(row.BonusType).Text
    },
    condition: {
      ko: db('GatheringCondition', null, true).findById(row.Condition).Text
    }
  } : undefined
}))
